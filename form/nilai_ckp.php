...
<div class="content">
            ...
            <tbody>
              <tr>
              <?php
                if($_SESSION['nip']=="198412142010031001"){
                  $sql = mysqli_query($conn,"SELECT DISTINCT kinerja_nip as nip,kinerja_nama as nama FROM `kinerja` INNER JOIN users ON users.nip = kinerja.kinerja_nip WHERE users.jabatan='Staff Sub Bagian Umum' ");
                } else if($_SESSION['nip']=="198212092010031001"){
                  $sql = mysqli_query($conn,"SELECT DISTINCT kinerja_nip as nip,kinerja_nama as nama FROM `kinerja` WHERE kinerja_nip = '199511142017012001' ");
                } else if($_SESSION['nip']=="197204041998031005"){
                  $sql = mysqli_query($conn,"SELECT DISTINCT kinerja_nip as nip,kinerja_nama as nama FROM `kinerja` WHERE kinerja_nip = '199504282017012001' ");
                } else if($_SESSION['nip']=="198711222012112001"){
                  $sql = mysqli_query($conn,"SELECT DISTINCT kinerja_nip as nip,kinerja_nama as nama FROM `kinerja` WHERE kinerja_nip = '197009241994012001' ");
                } else if($_SESSION['nip']=="196809151991121001"){
                  $sql = mysqli_query($conn,"SELECT DISTINCT kinerja_nip as nip,kinerja_nama as nama FROM `kinerja` INNER JOIN users ON users.nip = kinerja.kinerja_nip WHERE users.jabatan != 'Staff Sub Bagian Umum' AND users.nip != '199511142017012001' AND users.nip != '199504282017012001' AND users.nip != '197009241994012001'");
                }
                $id=1;
                if ($sql!= false && $sql->num_rows > 0) {
                  ...
                  while($row = $sql->fetch_assoc()) {
                        ...
                        $nama = $row['nama'];
              ?>
                        <td><a href="nilai_ckp_bulan.php?nama=<?= str_replace(" ","-",$nama); ?>"><?php echo $id++;?></a></td>
                        <td><a href="nilai_ckp_bulan.php?nama=<?= str_replace(" ","-",$nama); ?>"><?php echo $nip;?></a></td>
                        <td><a href="nilai_ckp_bulan.php?nama=<?= str_replace(" ","-",$nama); ?>"><?php echo $nama;?></a></td>
              </tr>
                    <?php
                      }
                }
