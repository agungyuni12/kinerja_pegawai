          ...
          <div class="container">
            <div class="row">
              <div class="col s12 m6 l6">
                <h5 class="breadcrumbs-title mt-0 mb-0"><span>MONITORING CKP <?= strtoupper($nama);?></span></h5>
              </div>
            </div>
          </div>
        </div>
            ...
            <tr>
                  <?php
                      if ($_SESSION['jabatan']=="admin") {
                        $id = 1;
                        $sql=mysqli_query($conn,"SELECT month(kinerja_tanggal) as bulan, year(kinerja_tanggal) as tahun FROM kinerja WHERE kinerja_nama='$nama' AND kinerja_kehadiran = 'HADIR' GROUP BY month(kinerja_tanggal),year(kinerja_tanggal) ORDER BY year(kinerja_tanggal),month(kinerja_tanggal)");
                        if ($sql!= false && $sql->num_rows > 0) {
                            ...
                            while($row = $sql->fetch_assoc()) {
                                ...
                                $sql2=mysqli_query($conn,"SELECT COUNT(DISTINCT DAY(kinerja_tanggal)) AS tersimpan, DAY(LAST_DAY(kinerja_tanggal)) AS total FROM kinerja WHERE kinerja_kehadiran='HADIR' AND kinerja_nama = '$nama' AND month(kinerja_tanggal) = '$bulan' AND year(kinerja_tanggal) = '$tahun'");
                                if ($sql2!= false && $sql2->num_rows > 0) {
                                  // output data of each row
                                  while($row2 = $sql2->fetch_assoc()) {
                                      $tersimpan = $row2['tersimpan'];
                                      $total = $row2['total'];
                                      $hari = $total-$tersimpan;
                        ?>
                        <td><?= $id++;?></td>
                        <td><?= $tahun;?></td>
                        <td><?= $nama_bulan;?></td>
