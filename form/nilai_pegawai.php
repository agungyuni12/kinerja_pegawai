            ...
            <tr>
              <?php
                if ($_SESSION['jabatan']!="admin") {
                  $sql = mysqli_query($conn,"SELECT users.nip as users_nip, users.nama_lengkap as users_nama, penilaian.nip_dinilai as penilaian_nip_dinilai, penilaian.nip_penilai as penilaian_nip_penilai FROM users LEFT JOIN penilaian ON users.nip = penilaian.nip_dinilai WHERE users.nama_lengkap != 'admin' AND users.nama_lengkap != 'Isa' AND users.nip != '$nip_penilai' ORDER BY users.nama_lengkap ");
                  $id=1;
                  if ($sql!= false && $sql->num_rows > 0) {
                    // output data of each row
                    while($row = $sql->fetch_assoc()) {
                          $nip_penilai2 = $row['penilaian_nip_penilai'];
                          $nip_dinilai1 = $row['penilaian_nip_dinilai'];
                          $nip_dinilai2 = $row['users_nip'];
                          $nama_dinilai = $row['users_nama'];
                                
                                if ($nip_penilai == $nip_penilai2 && $nip_dinilai1 == $nip_dinilai2) {
                                    ?>
                                        <td><a href="javascript:void(0);"><?php echo $id++;?></a></td>
                                        <td><a href="javascript:void(0);"><?php echo $nip_dinilai2;?></a></td>
                                        <td><a href="javascript:void(0);"><?php echo $nama_dinilai;?></a></td>
                                        <td>                                        
                                              <span class="chip purple lighten-5">
                                              <span class="purple-text">Tersimpan<span>
                                              </span>
                                        </td>
                                    <?php
                                } else {
                                  ?> 
                                        <td><a href="penilaian_pegawai.php?nama=<?= str_replace(" ","-",$nama_dinilai); ?>&nip=<?= str_replace(" ","-",$nip_dinilai2); ?>"><?php echo $id++;?></a></td>
                                        <td><a href="penilaian_pegawai.php?nama=<?= str_replace(" ","-",$nama_dinilai); ?>&nip=<?= str_replace(" ","-",$nip_dinilai2); ?>"><?php echo $nip_dinilai2;?></a></td>
                                        <td><a href="penilaian_pegawai.php?nama=<?= str_replace(" ","-",$nama_dinilai); ?>&nip=<?= str_replace(" ","-",$nip_dinilai2); ?>"><?php echo $nama_dinilai;?></a></td>
                                        <td>
